...
class BetaDogmaModel(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        logger.info("Initializing BetaDogmaModel")
    ...
    def _init_prediction_heads(self) -> None:
        ...
        for seq_head in [self.nmd_head, self.expression_head]:
            self._init_weights(seq_head)
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        if use_encoder_chunking:
            ...
            logger.debug("Processing full sequence with gradient checkpointing...")
    ...
    def _forward_encoder_chunked(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    def _forward_predictions_full(self, hidden_states: torch.Tensor) -> Dict[str, torch.Tensor]:
    ...
    def _forward_predictions_chunked_mps_safe(self, hidden_states: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        logger.debug("Strategy: Copy to CPU, slice on CPU, process chunks on MPS")
        ...
        logger.debug("Hidden states on CPU")
    ...
    def _forward_predictions_chunked(self, hidden_states: torch.Tensor) -> Dict[str, torch.Tensor]:
