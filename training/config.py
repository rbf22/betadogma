...
def get_gpu_config() -> dict:
    ...
    print(f"   Effective batch size: {cast(int, config['batch_size']) * cast(int, config['accumulate_grad_batches'])}")
    return config


def print_memory_breakdown(model: torch.nn.Module, optimizer: Optional[torch.optim.Optimizer] = None) -> None:
...
class Config:
    ...
    def _print_memory_strategy(self) -> None:
