...
class BetaDogmaDataset(Dataset):
    """Simplified dataset for DNA sequence prediction."""
    
    def __init__(
        self, 
        parquet_files: List[Path],
        tokenizer,
        max_seq_len: int = 450000,  # Default to max model length
        mode: str = "train",
        augment_prob: float = 0.0,  # Disable augmentation for now
        ...
        for f in self.parquet_files:
            try:
                with open(f, 'rb') as pf:
                    ...
                    print(f"  Found {num_rows:,} examples in {Path(f).name}")
        ...
        print(f"  Total: {self.length:,} examples")
