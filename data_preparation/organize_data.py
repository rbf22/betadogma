...
def run_command(cmd: List[str], description: str = None, check: bool = True, capture: bool = True):
...
def add_chr_prefix_vcf(input_file: Path, output_file: Path) -> bool:
    ...
    print(f"  Adding 'chr' prefix using bcftools...")
...
def sort_gtf(input_file: Path, output_file: Path) -> bool:
    ...
    print(f"    Sorting GTF file (this may take a few minutes)...")
    ...
    try:
        ...
        print(f"    Splitting headers and data...")
...
def process_file(src: Path, dst: Path, file_type: str, index: bool = True) -> bool:
    ...
    if dst.exists():
        ...
        if has_index:
            print(f"    ✓ Already processed with index")
            return True
    ...
    if has_chr is True:
        print(f"    ✓ Already has 'chr' prefix")
        ...
        # For FASTA, need to convert gzip to bgzip for indexing
            ...
            print(f"    Converting gzip to bgzip for indexing...")
        ...
        for ext in ['.tbi', '.csi', '.fai']:
            ...
            if idx_src.exists():
                ...
                print(f"    ✓ Copied existing index")
                index = False  # Don't re-index
    
    elif has_chr is False:
        print(f"    Converting chromosome names...")
        ...
        if file_type == "vcf":
            ...
            if file_type == "gtf":
                print(f"    Sorting GTF after chr prefix conversion...")
        ...
        print(f"    ⚠️  Could not determine chr prefix, copying as-is")
        shutil.copy2(src, dst)
...
def process_gtex_transcript_tpm():
    ...
    try:
        ...
        if sample_attrs.exists():
            ...
            sample_to_tissue = dict(zip(samples_df['SAMPID'], samples_df['SMTSD']))
