...
class TrainingDataGenerator:
    ...
    def __init__(
        ...
        splicevar_vcf: str,
        gtex_tpm: str,
        thousand_genomes_vcf: str = None,
        gtex_junctions: str = None,
        gtex_expression: str = None,
        output_dir: str = None,
        window_size: int = 300000,
        step_size: int = 150000,
        max_clinvar_variants: int = None,
        max_benign_variants: int = None,
        ...
        print(f"  VCF files ready for loading")
        ...
        for vcf_path, name in [
            ...
            if not vcf_path.exists():
                if name != "1000 Genomes":
                    print(f"  ❌ {name} VCF not found: {vcf_path}")
                continue
                
            if not tbi_path.exists():
                print(f"  ⚠️  Tabix index not found for {name}: {tbi_path}")
                self._create_tabix_index(vcf_path, name)
                continue
                
            if tbi_path.stat().st_mtime < vcf_path.stat().st_mtime:
                print(f"  ⚠️  Tabix index is older than VCF for {name}, recreating...")
                self._create_tabix_index(vcf_path, name)
                continue
                
            try:
                with pysam.TabixFile(str(vcf_path)) as tbx:
                    try:
                        contigs = tbx.contigs
                        if contigs:
    ...
    def _create_tabix_index(self, vcf_path: Path, name: str) -> None:
        ...
        try:
            ...
            print(f"    ✅ Successfully created tabix index")
            
        except Exception as e:
            print(f"    ❌ Failed to create tabix index: {e}")
            raise
    
    def _load_chromosome_variants(self, chrom: str):
    ...
    def _create_labels(self, region: Dict, window_start: int, seq_len: int) -> Dict:
        ...
        try:
            if not hasattr(self, '_chrom_transcripts'):
    ...
    def _save_batch(self, examples: List[Dict], split_name: str, chrom: str, batch_idx: int) -> None:
        ...
        try:
            ...
            output_dir = Path(self.output_dir) / split_name
    ...
    def print_summary(self) -> None:
        ...
        for split in ['train', 'val', 'test']:
            files = sorted((self.output_dir / split).glob("*.parquet"))
