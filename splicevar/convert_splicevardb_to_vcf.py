...
def write_vcf_header(f, source_file, build='hg38', chromosomes=None):
...
def load_and_parse_tsv(input_tsv, build='hg38'):
    """
...
def write_sorted_vcf(df, output_vcf, source_file, build='hg38'):
...
def convert_tsv_to_vcf(input_tsv, output_vcf, build='hg38', skip_mild=False, only_strong=False):
    """
    ...
    """
    print(f"{'='*80}")
    print(f"SpliceVarDB TSV to VCF Conversion")
    print(f"{'='*80}")
    ...
    df = load_and_parse_tsv(input_tsv, build)
    ...
    write_sorted_vcf(df, output_vcf, input_tsv, build)
