...
DEFAULT_CONFIG: NMDConfig = {
    "mode": "hybrid",  # Changed from "rule" to "hybrid"
    "ptc_rule_threshold": DEFAULT_PTC_THRESHOLD,
    "cds_inclusive": False,
    "strict_gt": True,
    "alpha": 0.5,  # Blend weight: 0.5 means equal contribution from rule and learned model
    "tau": 10.0,  # Softness parameter for the rule
    "hidden": 16,  # Hidden layer size for the MLP
    "init_bias": -1.5,  # Initial bias (predicts non-NMD by default)
}
...
class NMDModel(nn.Module):
    ...
    def _features(self, iso: Isoform) -> torch.Tensor:
        ...
        return torch.tensor(
            [float(n_exons), total_length / 1000.0, has_multiple_exons, last_exon_frac, soft_rule],
            dtype=torch.float32,
        )
    ...
    def predict(self, isoform: Isoform) -> float:
        ...
        if self.mode == "rule":
            ...
            return float(torch.tensor(1.0 if is_nmd else 0.0))
